...
/*!
 @brief ARM Assembly를 이용한 안티디버깅
 @discussion 라이브러리 후킹을 우회하는 어셈블리 레벨 구현
 */
void check_asm_debugger(void) {
    // ARM64 환경에서만 동작 (iOS 실제 디바이스)
    #ifdef __arm64__
    __asm__ volatile (
        "mov x0, #31\n"        // PT_DENY_ATTACH (first argument)
        "mov x1, #0\n"         // pid (0 = current process)
        "mov x2, #0\n"         // addr
        "mov x3, #0\n"         // data
        "mov x16, #26\n"       // ptrace syscall number
    ...
    NSLog(@"[AntiDebug] ARM64 Assembly 보호 활성화됨");
    #endif
    
    // ARM32 환경에서만 동작 (구형 디바이스)
    #ifdef __arm__
    __asm__ volatile (
        "mov r0, #31\n"        // PT_DENY_ATTACH
        "mov r1, #0\n"         // pid
        "mov r2, #0\n"         // addr
        "mov r3, #0\n"         // data
        "mov ip, #26\n"        // syscall number (ip = r12)
        "svc #80\n"            // supervisor call
    );
    NSLog(@"[AntiDebug] ARM32 Assembly 보호 활성화됨");
    #endif
    
    // x86_64 시뮬레이터용 (개발 시에만 사용)
    #ifdef __x86_64__
    NSLog(@"[AntiDebug] x86_64 시뮬레이터에서는 Assembly 보호 비활성화됨");
    #endif
}
